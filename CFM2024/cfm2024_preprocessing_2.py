...
for df in pd.read_csv(input_file, chunksize=chunk_size):
    ...
    # df['bid_distance'] = (df['mid_price']-df['bid'])/df['mid_price'] # mid price is often 0
    # df['ask_distance'] = (df['mid_price']-df['ask'])/df['mid_price'] # mid price is often 0
    ...
    new = df.groupby(['obs_id']).agg(
        ...
        mean_ask_dist=('ask_distance','mean'),
        mean_bid_dist=('bid_distance','mean'),
        std_rel_spread=('rel_spread','std'),
        min_rel_spread=('rel_spread','min'),
        max_rel_spread=('rel_spread','max'),
        mean_bid_ask_ratio=('bid_ask_ratio','mean'),
        std_bid_ask_ratio=('bid_ask_ratio','median'),
    ...
    new['price_range_pct'] = (new['max_price'] - new['min_price'])/new['median_price']
    new.drop(columns=['median_price','min_price','max_price'])
