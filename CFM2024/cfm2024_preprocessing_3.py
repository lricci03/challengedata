...
output_file = 'X_test_processed_4.csv'
chunk_size = 100000
...
for df in pd.read_csv(input_file, chunksize=chunk_size):
    ...
    df['relative_spread'] = df['spread']/np.clip(df['mid_price'], eps, None) # avoid dividing by 0
    ...
    new = df.groupby(['obs_id']).agg(
        ...
        median_relative_spread=('relative_spread','median'), # new feature
        # mean_bid_size=('bid_size','mean'),
        mean_bid_size_log=('bid_size', lambda x: np.mean(np.log1p(x))), # new feature
        median_bid_size=('bid_size','median'),
        # mean_ask_size=('ask_size','mean'),
        mean_ask_size_log=('ask_size', lambda x: np.mean(np.log1p(x))), # new feature
        ...
        mean_flux_log_abs=('flux', lambda x: np.mean(np.log1p(np.abs(x)))), # new feature
        unique_orders=('order_id','nunique'),
        mean_queue_depth = ('queue_depth_when_new','mean'),
        tick_size = ('price', lambda x: (
            x.drop_duplicates() # Keep only unique consecutive values
            .sort_values()      # <-- bug fix: must sort before diff().
            .diff()             # Calculate consecutive differences
            .abs()              # Convert to absolute values
            .loc[lambda diffs: diffs > 0]  # Filter out zeros (and NaNs)
            .min()                         # Extract the minimum nonzero difference
            )),
